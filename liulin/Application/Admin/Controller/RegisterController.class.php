...
class RegisterController extends Controller {
    ...
    public function index(){
        ...
        if($_POST){
            ...
            if ( !$liu_user->validate( $rule )->create() ) {
                    return $this->error( $liu_user->getError(),__ROOT__."/Admin/Register/" );
                }
            ...
            if( $isArr ){
                return $this->error( "用户名已存在",__ROOT__."/Admin/Register/" );
            }
            //判断两次密码输入是否一致
            if( $password != $password2 ){
                return $this->error( "两次密码输入不一致",__ROOT__."/Admin/Register/" );
            }
            ...
            if ( !$isArrT ) {
                return $this->error( "手机号或验证码错误",__ROOT__."/Admin/Register/" );
            }
            ...
            if( $uid>0 ){
                ...
                $liu_info->add( $ins );
                return $this->success( "注册成功",__ROOT__."/Admin/Admin/" );
            }else{
                return $this->error( "注册失败",__ROOT__."/Admin/Register/" );
            }
    ...
    public function Register_username(){
        ...
        $where['username']  = $username;
        //判断用户名是否已存在
        $isArr = $liu_admin->where( $where )->find();
        if( $isArr ){
            $data['status'] = 0;
        }else{
            $data['status'] = 1;
